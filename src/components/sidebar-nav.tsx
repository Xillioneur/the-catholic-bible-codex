...
export function SidebarNav() {
  ...
  const handleToggleDay = async (up: any, dayNumber: number, completed: boolean) => {
    if (!up || !up.planId) {
      console.error("[JOURNEY] Cannot toggle day: planId missing", { up, dayNumber, completed });
    ...
    try {
      ...
      if (completed) {
        ...
        // Advance to next day if sealing the current target day
                            ...
                            <div className="px-1 space-y-4">
                              ...
                              <div className="flex flex-col gap-3">
                                ...
                                {(() => {
                                  ...
                                  const diff = Date.now() - startedDate.getTime();
                                  ...
                                  if (calendarDay > (currentPlanProgress?.currentDay || 1)) {
                                    return (
                                      <button 
                                        onClick={() => {
                                          if (session) {
                                            updatePlanProgress.mutate({ planId: planDetails.id, currentDay: calendarDay });
                                          } else {
                                            if (currentPlanProgress.id) {
                                              void db.userReadingPlans.update(currentPlanProgress.id, { currentDay: calendarDay });
                                            }
                                          }
                                          toast.success(`Journey synced to Day ${calendarDay}`);
                                        }}
                                        className="w-full py-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[8px] font-black uppercase tracking-widest hover:bg-primary/5 hover:text-primary transition-all flex items-center justify-center gap-2 border border-zinc-200/50 dark:border-zinc-700/50"
                                      >
                                        <CalendarDays className="h-3 w-3" />
                                        Sync to Today (Day {calendarDay})
                                      </button>
                                    );
                                  }
                                  return null;
